<?php
// Iniciar sesión y limpiar cualquier salida previa para evitar errores en JSON
...
// --- ENDPOINT AJAX: Obtener registros de asistencia ---
if (isset($_GET['obtener_asistencias']) &&$_GET['obtener_asistencias'] == '1') {
...
// --- ENDPOINT AJAX: Generar nuevo código QR / Manual ---
...
// Obtener datos del profesor para la vista
try {
...
<head>
    ...
    <style>
        body { background-color: #0f111a; color: #e2e8f0; font-family: 'Segoe UI', system-ui, sans-serif; min-height: 100vh; }
        .navbar-docente { background-color: #1a1c29; border-bottom: 1px solid #2d3248; padding: 14px 28px; }
        .brand-logo { color: #a855f7; font-weight: 700; font-size: 1.25rem; letter-spacing: 1px; }
        .card-custom { background-color: #1a1c29; border: 1px solid #2d3248; border-radius: 16px; box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.5); }
        .banner-purple { background: linear-gradient(135deg, #6b21a8 0%, #3b0764 100%); border-radius: 12px; padding: 20px; color: #ffffff; margin-bottom: 20px; }
        .qr-container { background: #ffffff; padding: 16px; border-radius: 16px; display: inline-block; border: 2px solid #8b5cf6; }
        .codigo-alternativo { border: 2px dashed #8b5cf6; border-radius: 12px; padding: 16px; background-color: #131522; text-align: center; }
        .btn-purple { background: linear-gradient(135deg, #8b5cf6 0%, #6d28d9 100%); color: #ffffff; border: none; font-weight: 600; }
        .btn-purple:hover { background: linear-gradient(135deg, #7c3aed 0%, #5b21b6 100%); color: #ffffff; }
        .btn-outline-custom { border: 1px solid #3b4261; color: #94a3b8; background: transparent; }
        .btn-outline-custom:hover { background-color: #272a3d; color: #ffffff; }
        .table-dark-custom { color: #e2e8f0; --bs-table-bg: transparent; }
        .table-dark-custom thead th { background-color: #131522 !important; color: #a855f7 !important; border-bottom: 2px solid #2d3248 !important; padding: 14px 12px; }
        .table-dark-custom tbody td { background-color: transparent !important; color: #cbd5e1 !important; border-bottom: 1px solid #2d3248 !important; padding: 16px 12px; }
        .badge-purple { background-color: #3b0764; color: #d8b4fe; border: 1px solid #6b21a8; }
        .time-badge { color: #a855f7; font-weight: 600; }
        .text-subtle { color: #94a3b8 !important; }
        .timer-badge { background-color: rgba(168, 85, 247, 0.15); color: #c084fc; border: 1px solid #8b5cf6; font-size: 0.85rem; padding: 4px 12px; border-radius: 20px; }
    </style>
</head>
<body>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            function actualizarReloj12H() {
                ...
                if (relojEl) relojEl.textContent = `${String(horas).padStart(2, '0')}:${minutos} ${ampm}`;
            }
                            ...
                            result.data.forEach(item => {
                                ...
                                html += `
                                    <tr>
                                        ...
                                        <td class="text-light">${item.grado}</td>
        ...
        async function forzarNuevoCodigo() {
            try {
                tiempoRestante = 300;
                ...
                if (data.success) {
                    ...
                    document.getElementById('qr-imagen').src = `https://api.qrserver.com/v1/create-qr-code/?size=190x190&data=${encodeURIComponent(data.datos_qr)}`;
